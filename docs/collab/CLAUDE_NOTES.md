...
## 0-quater. Быстрые фичи + Постоянные заказы v1 — 2026-07-24 (по прямым поручениям владельца)

Работал оркестратором с суб-агентами (admin-frontend, flutter-dev) — по разрешению владельца.

**Быстрые фичи (commit `26ef184`, версия 1.0.3+4 `c67dd3e`, APK sha `c573c69e…`):**
- Удаление сотрудников: owner-only `DELETE /staff/{id}` (владельца нельзя, hard-delete
  отзывает доступ сразу — `get_current_staff` перечитывает БД, email освобождается) + кнопка
  в `/staff`. Отдельно от «отключить».
- Кликабельный бейдж «Недоступно N товаров» → модалка со списком (admin, чистый фронт:
  `available_branch_ids` уже был).
- Лого в шапке: полноразмерный источник вместо миниатюры + decode под DPR
  (`cacheWidth`/`filterQuality`) + 40→48. Причина «замыленности» — использовалась миниатюра.
- Проверки: admin typecheck, backend 96/96, Flutter 99/99.

**Постоянные заказы v1 (дизайн: `docs/design/RECURRING_ORDERS_V1.md`, версия 1.0.4+5):**
Раньше подписка была только сохранённой предоплатой — НЕ создавала заказы. Теперь:
- Backend-ядро (`931d983`): новый модуль `recurring.py` — планировщик (asyncio-таск в
  lifespan, тик 60с) генерирует `scheduled`-заказы на сегодня ЗАРАНЕЕ и активирует
  `scheduled→new` за 10 мин до времени (Asia/Bishkek UTC+6). Идемпотентность в БД:
  UNIQUE `(company_id, recurring_order_id, service_date)`. Решает час-пик (план виден заранее,
  не «внезапные» заказы). Новый `PATCH /customer/me/recurring` — правка состава/времени/
  филиала/пожеланий БЕЗ сброса `paid_until` (PUT остаётся покупкой/продлением). Серверный
  `dailyTotal` — честная цена по текущему каталогу. `OrderOut.isRecurring/scheduledFor`,
  статус `scheduled` (только →new/cancelled). Миграция `e5a7c3d19b60`. Тесты 100/100.
- Push-backend (`5ccce95`): FCM HTTP v1 (`push.py`, fail-closed без кредов — честный no-op),
  токены устройств `customer_push_tokens` + upsert/remove-ручки (токен=устройство,
  переприсваивается при смене аккаунта), пуш при активации заказа. Миграция `f7b9d4e82c15`.
  Тесты 103/103.
- Admin (агент, `aad1194`): панель «Расписание постоянных» (виден план дня), бейджи
  «Постоянный · к HH:MM», `scheduled` исключён из счётчиков/доски. typecheck + 15/15.
- Flutter (агент, `3af89a0`): редактирование подписки (PATCH, без оплаты) + пожелания +
  серверная цена + метка «Постоянный заказ» в истории. analyze + 101/101.
...
**ДЕПЛОЙ ВЫПОЛНЕН И ПРОВЕРЕН 2026-07-24** (владелец прогнал runbook, я сверял вывод):
- Миграция применена штатно: `alembic ... Running upgrade c19f6b4a8e21 -> d8e42c1a7f90` — ровно
  одна, как и предсказывал граф. `alembic current` до апгрейда = `c19f6b4a8e21` (подтвердилось).
- `backend` и `admin` пересобраны из архива `5e9fb34`, оба `Healthy`; `nginx` пересоздан; все 5
  сервисов Up. Prod smoke: `/ready`=200, `/login`=200, company config=200, `/admin`=302,
  `/media/temp/`=403.
- APK `1.0.2+3` разложен в `/srv/sweetime/downloads/SweetTime.apk`, sha256 совпал
  (`3c71119f…7cadc1`). Грабли: каталог downloads принадлежит root → копия только через `sudo`;
  в runbook порядок шагов исправлен (smoke ДО копии APK) — commit `c361073`.
- Runbook приведён к привычному владельцу стилю (архив + guard `.env`), commit `8f7ff2c`.
- ОСТАЛОСЬ (только человек): браузерная приёмка ролей owner/manager/barista; на устройстве —
  одна image и одна video отложенная публикация; SMTP при появлении реальных кредов. После
  приёмки владельцем — двигать статусы в `docs/TASKS.md`.
...
### CL-009 — приёмка и коммит V2 Codex + доводка выкатки (2026-07-27)

Владелец попросил проверить работу Codex и доделать. Твой (Codex) Recurring Orders V2 приняты:
- **Закоммитил всю твою незакоммиченную работу** (было +4011 строк/7 новых файлов вне git — риск):
  `75d7b7d` backend (recurring_api/refunds/3 миграции), `0e8f2f3` admin (registry+dashboard),
  `7405c71` flutter (несколько подписок), `61a7b67` docs, `36dfa52` bump 1.0.8+9. Твой журнал и
  `.codex-phone-install.png` не редактировал.
- **Починил 1 падавший Flutter-тест** «recurring V2 PATCH … preserves omitted fields»: устаревший
  литерал — фейк с фичей custom_until стал писать `customUntil:null`, а ожидаемая карта его не
  содержала. Это НЕ прод-баг: реальный `?_dateOnly` опускает null из HTTP-тела. Дописал
  `customUntil:null` в ожидаемую карту (widget_test.dart:769). Твоя моя правка наблюдаемости
  планировщика в `recurring.py` уцелела — спасибо, что объединил.
- **Проверки после фикса:** backend 122, Flutter 109, admin typecheck + content-тесты — все зелёные.
- **Нашёл незавершённость выкатки:** твой rollout-архив содержал только миграцию `9d3f1c7a2b60`,
  а голова — `d2f6a8b94c31` (ещё refunds `4a6e2d9c81f0` + custom_until). Прод стоял на промежуточной
  ревизии. Собрал полный архив `dist/sweettime-recurring-v2-36dfa52.tar.gz` (sha `1060d8f9…`, все 3
  миграции) и APK 1.0.8+9 (sha `9ab1c3f1…`, поставлен на f3bff2a5). Владельцу дал шаги домиграции
  прода `9d3f1c7a2b60 → d2f6a8b94c31` + передеплой. Осталось: выполнить это + ручная приёмка
  (несколько подписок, edit/reprice/cancel, дашборд под owner).
